# # Relaxing Music Player
An app with relaxing songs. Perfect to listening while studying

[View Live](https://gabrielamcarvalho.github.io/RelaxingMusicPlayer/)

### Tools

:grey_exclamation: Icons: [Fontawesome](https://fontawesome.com/)

:hammer:Images  : [Unsplash](https://unsplash.com/)

:smile: Songs : [Bensound](https://www.bensound.com/)

*This project is part of the [Zero to Master couse](https://academy.zerotomastery.io/p/javascript-projects). Project 8 of 20.*
